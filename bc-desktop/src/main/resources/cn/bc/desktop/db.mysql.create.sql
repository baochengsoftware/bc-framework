...
-- 个人设置
create table BC_DESKTOP_PERSONAL_CONFIG (
    ID int NOT NULL auto_increment,
    UID_ varchar(36) COMMENT '全局标识',
    STATUS_ int(1) NOT NULL COMMENT '状态：0-已禁用,1-启用中,2-已删除',
    INNER_ int(1) NOT NULL COMMENT '是否为内置对象:0-否,1-是',
...
ALTER TABLE BC_DESKTOP_PERSONAL_CONFIG ADD CONSTRAINT FK_PERSONAL_ACTOR FOREIGN KEY (AID) 
	REFERENCES BC_IDENTITY_ACTOR (ID);
ALTER TABLE BC_DESKTOP_PERSONAL_CONFIG ADD UNIQUE INDEX PERSONAL_AID_UNIQUE (AID) ;